...
/*
* генератор формы
* string/array $$item - порядковый номер, диапазон или массив номеров.
* Пример: $item=1/$item='1-3'/$item=array(1,3,6,5);
* string $form_settings - массив в  config/form_validation.php
* array $wrappers массив с обертками для полей формыю
* $wrappers['form_field_wrappers']='<div class="item"></div>';//обертка всего поля формы(label+input+error)
* $wrappers['label_wrappers']='<label class="label"></label>';//вид элемента формы label. В обертку будет подставлен атрибут  for
...
* object/array $default - объект/массив со значениями по умолчанию
* */
function render_item_form($item = array(), $form_settings = null, $wrappers = array(), $data = array(), $default = null)
{
    if (empty($item)) return 'Не указан параметр $item';

    //если $item - число
    if (is_numeric($item)) $result = (array)$item;
    //если $item - диапазон (0-5)
    ...
    //если $item - массив array(0,2,3,5)
    if (is_array($item)) $result = $item;
    if (empty($result)) return 'Не верно указан параметр $item. $item - число или диапазон или массив';
    ...
    {
        if(!empty($form_settings[$value]))
        array_push($finish, $form_settings[$value]);
    }
    render_form($finish, $wrappers, $data, $default);
